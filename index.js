...
const keywordsCategories = [
    {
        "keywords": ["Javascript", "Promises", "React", "Vue JS", "Angular", "Java", "ES6"]
    },
    {
        "keywords": ["Lecture", "Livres", "Conseils librairie", "Javon", "Bibliothèque"]
    },
    {
...
const cleanedKeyword = (keyword) => {
    ...
    cleanedKeyword = cleanedKeyword.replace(/['éèê']/g, 'e').replace(/['à']/g, 'a').replace(/['ô']/g, 'o');
    cleanedKeyword = cleanedKeyword.replace(/[^a-zA-Z0-9 ]/g, "").toLowerCase();
...
const reloadArticles = () => {
    document.querySelector(".articlesList").innerHTML = ""
    let articlesToShow = data.articles.filter(n => { for (keyword of currentKeywords) {
	                                                 if (n.tags.includes(keyword)) 
	                                                     return true
                                                      }
	                                              return false
                                                    });
...
const showKeywordsList = (value) => {
	...
	//possibleKeywords = allKeywords.filter(k => k.startsWith(value));
	 possibleKeywords = [];
	 for (element of keywordsCategories) {
             for (keyword of element.keywords) {
	     	if (cleanedKeyword(keyword).startsWith(cleanedKeyword(value))) { 
		     possibleKeywords.push(keyword);
		     possibleKeywords = possibleKeywords.concat(element.keywords.filter(k => k != keyword));
		     break;
		}
	     }
	 }

	 for (keyword of possibleKeywords) 
	    keyWordUl.innerHTML += `
